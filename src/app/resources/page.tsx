...
const ResourcesPage = () => {
  ...
  return (
    <main className="min-h-screen bg-brand-light">
      ...
      {/* Header — brand-light with white filter bar */}
      <section className="pt-32 pb-16 bg-white relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center relative z-10">
          ...
          <h1 className="text-heading-lg lg:text-display font-black text-brand-blue uppercase tracking-tight mb-4">
            Tax Kavach <span className="text-brand-grey">Resources</span>
          </h1>
          <p className="text-body-lg text-brand-grey font-medium max-w-2xl mx-auto mb-10">
            Professional insights, regulatory updates, and expert strategies for a complex tax landscape.
          </p>

          {/* Filter Bar — Each button has clear affordance with active/inactive states */}
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`px-6 py-3 rounded-xl font-bold text-xs uppercase tracking-wider transition-all duration-200 min-h-[44px] ${
                  ...
                  : 'bg-brand-light text-brand-grey border border-slate-200 hover:border-brand-blue hover:text-brand-blue'
                }`}
              >
                {cat}
              </button>
      ...
      <section className="py-16 lg:py-20 relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {filteredPosts.map((post) => (
                <Link key={post.id} href={`/resources/${post.slug || post.id}`} className="group">
                  <article className="h-full flex flex-col bg-white rounded-3xl border border-slate-100/80 overflow-hidden shadow-elevation-1 card-interactive">
                    <div className="aspect-[16/10] bg-slate-100 relative overflow-hidden">
                      {post.imageUrl ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-brand-blue/5 to-brand-blue/10 text-brand-blue/15">
                      ...
                      <div className="absolute top-4 left-4 px-3 py-1.5 bg-brand-blue text-white text-[10px] font-bold uppercase tracking-[0.15em] rounded-lg shadow-lg">
                        {post.category}
                      </div>
                    </div>
                    <div className="p-8 flex flex-col flex-grow">
                      <h2 className="text-xl font-bold text-brand-blue mb-3 leading-tight group-hover:text-brand-blue-dark transition-colors">
                        {post.title}
                      </h2>
                      <p className="text-brand-grey font-medium line-clamp-3 mb-6 flex-grow text-body leading-relaxed">
                        {post.metaDescription || "Expert insights on modern tax compliance and litigation strategies from the Tax Kavach team."}
                      </p>
                      <div className="pt-5 border-t border-slate-100 flex items-center text-xs font-bold text-brand-blue uppercase tracking-widest">
                        Read Full Insight 
          ...
          {!loading && filteredPosts.length === 0 && (
            <div className="text-center py-16 bg-white rounded-3xl border-2 border-dashed border-slate-200 shadow-elevation-1">
              <div className="text-5xl mb-4 grayscale opacity-30">📰</div>
              <p className="text-brand-grey font-bold text-lg">No articles found in this category yet.</p>
              <p className="text-brand-steel text-sm mt-1">Stay tuned for expert insights!</p>
            </div>
          )}
        </div>
      </section>
